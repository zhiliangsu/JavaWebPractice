...
(
    ...
    type        tinyint unsigned             not null comment '类型, 菜品分类: 1 套餐分类: 2',
...
(
    ...
    image       varchar(300)  not null comment '菜品图片url',
    description varchar(200)  null comment '菜品描述',
    status      tinyint       not null comment '菜品售卖状态: 0 停售, 1 起售',
    create_time datetime      not null comment '创建时间',
    update_time datetime      not null comment '修改时间',
    constraint name
        unique (name)
...
(
    ...
    image       varchar(300)                 not null comment '套餐图片url',
